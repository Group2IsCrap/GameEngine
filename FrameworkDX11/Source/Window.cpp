...
namespace TempEngineNamespace
{
	...
	bool Window::Initialise(WindowContainer* windowContainer, HINSTANCE hInstance, const char* windowTitle, std::string windowClass, int width, int height)
	{
		...
		mWindowTitleWide = Utils::StringHelpers::StringToWide(windowTitle);
		mWindowClass = windowClass;
		mWindowClassWide = Utils::StringHelpers::StringToWide(windowClass);
		mWidth = width;
		mHeight = height;

		RegisterWindowClass();

		int screenCentreX = GetSystemMetrics(SM_CXSCREEN) / 2 - mWidth / 2;
		int screenCentreY = GetSystemMetrics(SM_CYSCREEN) / 2 - mHeight / 2;
		...
		mHandle = CreateWindowEx(0,                   // Default style
			...
			mHInstance,                               // Module instance for window
			windowContainer);
	...
	void Window::RegisterWindowClass()
	{
		...
		windowClass.lpszClassName = mWindowClassWide.c_str();
