...
AIBearEntity::AIBearEntity() : AIEntity()
{
	GetComponent<SpriteComponent>()->texture = Firelight::Graphics::AssetManager::Instance().GetTexture("Sprites/Enemies/BearIdle.png");
}

AIBearEntity::AIBearEntity(Firelight::ECS::EntityID entityID) : AIEntity(entityID)
{
}

AIBearEntity::AIBearEntity(bool isTemplate, Firelight::ECS::EntityID entityID) : AIEntity(isTemplate, entityID)
{
	GetIDComponent()->name = "Bear";
	//GetComponent<RigidBodyComponent>()->nextPos = { 2.0f, 0.0f, 0.0f };
	GetSpriteComponent()->texture = Firelight::Graphics::AssetManager::Instance().GetTexture("Sprites/Enemies/BearIdle.png");
	...
	AIStateBehaviour* attackingBehaviour = new AIAttackingBehaviour(GetEntityID(), GetRigidBodyComponent(), GetAIComponent(), "BearIdle", "BearWalk", 2, 0.7f, 2.0f, 2.0f, 1.0f, "Crocodile Attack.wav");
